...
| `api` | OpenAI 兼容 HTTP 入口：SSE 透传/模拟流、心跳、错误事件；`cmd/qfy-agent-server` 为可运行示例 |
...
模型注册表为 YAML（示例见 `config/models.example.yaml`）：
...
go run ./cmd/qfy-agent-server -config config/models.example.yaml -addr 127.0.0.1:8080
```
